...
class Withdraw extends Value
{
    ...
    public function calculate(NovaRequest $request)
    {
		...
		if($request->user()->access != 'administrator') {
			...
			$result = withdrawAndDeposit::whereIn('operator', $apioperator)->selectRaw('SUM(deposit) as total')
				->whereBetween('created_at', $this->currentRange($request->range, $timezone))
				->toBase()
				->first();
			$previous = withdrawAndDeposit::whereIn('operator', $apioperator)->selectRaw('SUM(deposit) as total')
			...
			$result = withdrawAndDeposit::selectRaw('SUM(deposit) as total')
				->whereBetween('created_at', $this->currentRange($request->range, $timezone))
				->toBase()
				->first();
			$previous = withdrawAndDeposit::selectRaw('SUM(deposit) as total')
				->whereBetween('created_at', $this->previousRange($request->range, $timezone) )
				->toBase()
				->first();	
			return $this->result($result->total/100)->previous($previous->total/100)->currency('$')->format('0,0');
