...
class WithdrawTrend extends Trend
{
    /**
    ...
    public function calculate(NovaRequest $request)
    {
        if($request->user()->access == 'administrator') {
			return $this->sumByDays($request, withdrawAndDeposit::class, DB::raw('(deposit/100)'))->format('$0,0');
		} else {
			...
			return $this->sumByDays($request, withdrawAndDeposit::whereIn('operator', $apioperator), DB::raw('(deposit/100)'))->format('$0,0');
		}
    }
